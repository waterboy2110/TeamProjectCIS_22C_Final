...
int Hash::hashFunction(int tNum)
{
    ...
    // If the sum is only one digit long...
    if (sumString.length() == 1)
    {
        ...
        {
            // If the sub-sum is only one digit long
            ...
            // If the sum is not divisible by two and the
            // sub-sum has a length greater than two...
            subSumString.append(subSumString);
            return atoi(sumString.c_str());
        }
    }
    
    // If the sum is greater than one digit in length...
    return (sum+subSum)/2;
...
bool Hash::addHash(int addNum, restaurantInfo *addRestaurant)
{
    ...
    if (hashAryPtr[addHashNum].numRestaurants == 0)
    {
        ...
        // Update total number of restaurants
        totalRestaurants++;
...
void Hash::printInKeySequence()
{
    ...
    while (count < hashSize)
    {
        ...
        if (hashAryPtr[count].numRestaurants == 1)
        {
            ...
            //cout << "\nDISPLAY REST.";
            //hashAryPtr[count].aRestaurant->displayRestaurant();
        ...
        if (hashAryPtr[count].numRestaurants > 1)
        {
            ...
            while (colCount < hashAryPtr[count].numRestaurants - 1)
            {
                ...
                //cout << "\nDISPLAY REST.";
                //tempArray[countP].displayRestaurant();
...
//***********************************************************************************************************
// Definition of displayNumRestaurants
//
//***********************************************************************************************************
void Hash::displayNumRestaurants() const
{
    cout << "\nTotal number of restaurants: " << totalRestaurants;
}
